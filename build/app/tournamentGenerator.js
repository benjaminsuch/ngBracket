  ...
  .factory('tournamentGenerator', function() {
    return {
      newTournament: function(ttype, teams, playBronzeMatch, doubleConference) {
         ...
         *
         * @description
         * Creates a match scaffold with a match id "match-<conference>-<roundNumber>-<matchNumber>".
         *
         * @param {string|number} roundNumber
         * @param {string|number} matchNumber
         * @param {string|number} conference
         *
        ...
        function generateRound(teams, roundNumber, tournamentData, conference) {
          ...
          if (roundNumber === 1) {
            ...
            if (excessParticipants > 0) {
              ...
              var startIndex;
              console.log('shiftedMatches', shiftedMatches);
              console.log('closestBalancedTree', closestBalancedTree);
              if (shiftedMatches === 0) {
                startIndex = closestBalancedTree;
              } else if (shiftedMatches > 0) {
                startIndex = closestBalancedTree + (shiftedMatches * 2);
              } else {
                startIndex = closestBalancedTree - (Math.abs(shiftedMatches) * 2);
              }
          ...
          for (i = 0; i < teams.length; i++) {
            ...
            if (roundNumber === 1) {
              // normal brackets until reaching biggest possible balanced tree...
              if (i % 2 !== 0) {
                continue;
              }
         ...
         *
         *
         * @param {string} ttype Possible values are "SE" or "DE" and define how the bracket is played out.
         * @param {Array} teams A list of teams to apply on the bracket. The teams are stored as objects in this list.
         * @param {boolean} playBronzeMatch Set to true if the 3rd place get played out.
         * @param {string} conference Name of the conference.
         *
         * @returns {{type, conferences, properties}|*}
         */
        function createTournament(ttype, teams, playBronzeMatch, conference) {
          // Stores the teams of the previous round
          var previousRound = [];
          var loserBracket = [];
          var roundNumber = 1;
          ...
          // Create new rounds until there is only one match, the finals, left.
          while (roundNumber === 1 || Math.floor(previousRound.length) > 1) {
            var partic = previousRound;

            if (roundNumber === 1) {
              // Create a copy of teams by using .slice();
              partic = teams.slice();
            }
         ...
         *
         * @description
         * Creates a default conference object.
         *
         * @returns {object} New conference object
         */
